...
public class CableSerialPort {
    ...
    private boolean prepare() {
		disconnect();  // 每次 prepare 都先完全清理掉舊的連線
		Log.d(TAG, "prepare: finished disconnect, start prepare new connection");
    ...
    public boolean connect() {
		...
		if (onStateChanged != null) {
			onStateChanged.onRunError("Prepare failed: unable to find USB device or driver");
		}
    ...
    public void disconnect() {
		...
		if (onStateChanged != null) {
			try {
				...
				Log.e("CableSerialPort", "Error in onDisconnected: " + e.getMessage());
			}
		}
		...
		if (usbIoManager != null) {
			try {
				...
				Log.e("CableSerialPort", "Error stopping usbIoManager: " + e.getMessage());
			} finally {
				usbIoManager = null;
			}
		}
		...
		if (usbSerialPort != null) {
			try {
				...
				Log.e("CableSerialPort", "Error closing usbSerialPort: " + e.getMessage());
			} finally {
				usbSerialPort = null;
			}
		}
		...
		if (usbConnection != null) {
			try {
				...
				Log.e("CableSerialPort", "Error closing usbConnection: " + e.getMessage());
			} finally {
				usbConnection = null;
			}
		}
    ...
    public static class SerialPort {
		...
		public UsbDevice usbDevice = null; // <--- BV6LC 新增: 保留 UsbDevice 完整資訊
		

        public SerialPort(int deviceId, int vendorId, int productId, int portNum, UsbDevice usbDevice) {
        ...
        public String information() {
			 if (usbDevice != null) {
				String manufacturer = "X";
				try {
					manufacturer = safeString(usbDevice.getManufacturerName());
				} catch (Exception e) {
					manufacturer = "X";
				}
				...
				String product = "X";
				try {
					product = safeString(usbDevice.getProductName());
				} catch (Exception e) {
					product = "X";
				}
				...
				String serial = "X";
				try {
					serial = safeString(usbDevice.getSerialNumber());
				} catch (Exception e) {
					serial = "X";
				}

				return String.format("%s %s (VID: %04X, PID: %04X, SN: %s)",
						manufacturer, product, vendorId, productId, serial);
				//return String.format("%s %s (PID: %04X, SN: %s)",
				//		manufacturer, product, productId, serial);				
