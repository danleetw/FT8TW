...
public class Js8Message {
    private static String TAG = "Js8Message";
    public int i3 = 0;
    public int n3 = 0;
	...
	public boolean isFollow_Callsign = false;// 是否為關注呼號
	
	public boolean isPotaSota = false;// 是否為SOTA/POTA
	...
	public boolean hasCheckedFromLocation = false;
     ...
     * @param signalFormat
     */
    public Js8Message(int signalFormat) {
		
        //this.signalFormat = signalFormat;
		
		this.signalFormat = GeneralVariables.isFT4 ? FT8Common.FT4_MODE : FT8Common.FT8_MODE;
    }

    public Js8Message(String callTo, String callFrom, String extraInfo) {
    ...
    public Js8Message(int i3, int n3, String callTo, String callFrom, String extraInfo) {
     ...
     */
    public Js8Message(Js8Message message) {
		
        if (message != null) {
            //signalFormat = message.signalFormat;
			signalFormat = GeneralVariables.isFT4 ? FT8Common.FT4_MODE : FT8Common.FT8_MODE;
            ...
            if (message.i3 == 4) {
				int[] hashes = Js8DecodedMessage.computeHashes(message.callsignFrom);
				...
				hashList.addHash(hashes[0], message.callsignFrom);
				hashList.addHash(hashes[1], message.callsignFrom);
				hashList.addHash(hashes[2], message.callsignFrom);
            }
			...
			this.modifier="";
			if (callsignTo != null) {
				//Pattern pattern = Pattern.compile("^CQ\\s+(.+)", Pattern.CASE_INSENSITIVE);
				Pattern pattern = Pattern.compile("^CQ\\s+(SOTA|POTA|OTA)\\b", Pattern.CASE_INSENSITIVE);
				Matcher matcher = pattern.matcher(callsignTo.trim());
				if (matcher.find()) {
					//this.modifier=matcher.group(1).trim();
					isPotaSota=true;
				}
			}
			
			//isPotaSota=modifier.equalsIgnoreCase("POTA") || modifier.equalsIgnoreCase("SOTA");
    ...
    public String getMessageText() { 
		//LogExt.d(TAG , "getMessageText i3="+i3 +" n3:"+n3+" modifier:"+modifier+ " extraInfo:"+extraInfo +" callsignTo:"+callsignTo+" callsignFrom:"+callsignFrom);
...
*/
    ...
    public String getModifier() {		
        if (signalFormat == FT8Common.FT8_MODE) {
			if (modifier != null) {
				return modifier.replace("<", "").replace(">", "");
			}
		}
		return "format:"+signalFormat+" "+modifier;
    }
    ...
    public static String getCommandInfoByI3N3(int i, int n) {
        ...
        switch (i) {
            case 1:
            case 2:
                return String.format(format, i, 0, GeneralVariables.getStringFromResource(R.string.std_msg)); //標準訊息
            case 5:
                return String.format(format, i, 0, "EU VHF");
            case 3:
                return String.format(format, i, 0, GeneralVariables.getStringFromResource(R.string.rtty_ru_msg)); //RTTY RU
            case 4:
                return String.format(format, i, 0, GeneralVariables.getStringFromResource(R.string.none_std_msg)); //非標準訊息
            case 0:
                switch (n) {
                    case 0:
                        return String.format(format, i, n, GeneralVariables.getStringFromResource(R.string.free_text)); //自定義訊息
                    case 1:
                        return String.format(format, i, n, GeneralVariables.getStringFromResource(R.string.dXpedition)); //遠征
                    case 3:
                    case 4:
                        return String.format(format, i, n, GeneralVariables.getStringFromResource(R.string.field_day)); //野外日
                    case 5:
                        return String.format(format, i, n, GeneralVariables.getStringFromResource(R.string.telemetry)); //遙測
                }
        }
        return "";
    }
	...
	public void printAllFields() 
	{
		...
		LogExt.d(TAG,"========== Ft8Message Info ==========");
		LogExt.d(TAG,"UTC Time: " + utcTime + " (" + UtcTimer.getDatetimeStr(utcTime) + ")");
		LogExt.d(TAG,"Valid: " + isValid);
		LogExt.d(TAG,"SNR: " + snr);
		LogExt.d(TAG,"Time Offset (sec): " + time_sec);
		LogExt.d(TAG,"Frequency (Hz): " + freq_hz);
		LogExt.d(TAG,"Score: " + score);
		LogExt.d(TAG,"Message Hash: %#06X\n"+ messageHash);
		LogExt.d(TAG,"Callsign From: " + callsignFrom);
		LogExt.d(TAG,"Callsign To: " + callsignTo);
		LogExt.d(TAG,"Modifier: " + modifier);
		LogExt.d(TAG,"Extra Info: " + extraInfo);
		LogExt.d(TAG,"Maiden Grid: " + maidenGrid);
		LogExt.d(TAG,"RTTY State: " + rtty_state);
		LogExt.d(TAG,"R Flag: " + r_flag);
		LogExt.d(TAG,"RTTY TU: " + rtty_tu);
		LogExt.d(TAG,"EU Serial: " + eu_serial);
		LogExt.d(TAG,"ARRL RAC: " + arrl_rac);
		LogExt.d(TAG,"ARRL Class: " + arrl_class);
		LogExt.d(TAG,"DX Call To2: " + dx_call_to2);
		LogExt.d(TAG,"Report: " + report);
		LogExt.d(TAG,"CallFrom Hash10: " + callFromHash10);
		LogExt.d(TAG,"CallFrom Hash12: " + callFromHash12);
		LogExt.d(TAG,"CallFrom Hash22: " + callFromHash22);
		LogExt.d(TAG,"CallTo Hash10: " + callToHash10);
		LogExt.d(TAG,"CallTo Hash12: " + callToHash12);
		LogExt.d(TAG,"CallTo Hash22: " + callToHash22);
		LogExt.d(TAG,"Band: " + band);
		LogExt.d(TAG,"From Where: " + fromWhere);
		LogExt.d(TAG,"To Where: " + toWhere);
		LogExt.d(TAG,"Is QSL Callsign: " + isQSL_Callsign);
		LogExt.d(TAG,"Is Follow Callsign: " + isFollow_Callsign);
		LogExt.d(TAG,"Is POTA/SOTA: " + isPotaSota);
		LogExt.d(TAG,"From DXCC: " + fromDxcc);
		LogExt.d(TAG,"From ITU: " + fromItu);
		LogExt.d(TAG,"From CQ: " + fromCq);
		LogExt.d(TAG,"To DXCC: " + toDxcc);
		LogExt.d(TAG,"To ITU: " + toItu);
		LogExt.d(TAG,"To CQ: " + toCq);
		LogExt.d(TAG,"Has Checked From Location: " + hasCheckedFromLocation);
		LogExt.d(TAG,"From LatLng: " + (fromLatLng != null ? fromLatLng.toString() : "null"));
		LogExt.d(TAG,"To LatLng: " + (toLatLng != null ? toLatLng.toString() : "null"));
		LogExt.d(TAG,"Is Weak Signal: " + isWeakSignal);
		LogExt.d(TAG,"i3: " + i3 + ", n3: " + n3);
		LogExt.d(TAG,"Signal Format: " + signalFormat);
		LogExt.d(TAG,"Message Text: " + getMessageText());
		LogExt.d(TAG,"Command Type: " + getCommandInfo());
		LogExt.d(TAG,"======================================");
	}
