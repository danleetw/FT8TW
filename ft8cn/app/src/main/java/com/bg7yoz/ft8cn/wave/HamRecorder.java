...
import com.bg7yoz.ft8cn.util.MemoryWatch;
...
public class HamRecorder {
	...
	// 循環錄音 (JS8)
	private static final int CIRCULAR_SECONDS = 20;
	private static final int CIRCULAR_SIZE = sampleRateInHz * CIRCULAR_SECONDS; // 240000
	private final float[] circularBuffer = new float[CIRCULAR_SIZE];
	private int circularWritePos = 0;
	// 循環錄音 js8call
    ...
    public void doOnWaveDataReceived(int bufferLen,float[] buffer){
		...
		MemoryWatch.check(); // <-- 每筆音訊進來先檢查記憶體
		
		
		// -------------循環錄音
		// 寫入循環錄音 circular buffer
		if(GeneralVariables.js8call){
			LogExt.d(TAG, "寫入循環錄音...###");
			for (int i = 0; i < bufferLen; i++) {
				circularBuffer[circularWritePos] = buffer[i];
				circularWritePos = (circularWritePos + 1) % CIRCULAR_SIZE;
			}
		}
	...
	// 讀取循環錄音
	public float[] getLatestAudioSegment(int seconds) {
		int segmentLen = seconds * sampleRateInHz; // 13 * 12000
		
		if (segmentLen > CIRCULAR_SIZE) {
			Log.e(TAG, "要求讀取長度超過環狀錄音緩衝區大小");
			segmentLen = CIRCULAR_SIZE; // 自動降為最大長度
		}
		
		
		
		float[] output = new float[segmentLen];
		int readPos = (circularWritePos - segmentLen + CIRCULAR_SIZE) % CIRCULAR_SIZE;

		for (int i = 0; i < segmentLen; i++) {
			output[i] = circularBuffer[readPos];
			readPos = (readPos + 1) % CIRCULAR_SIZE;
		}
		return output;
	}
	//float[] latest13s = hamRecorder.getLatestAudioSegment(13);
	//Ft8DecodedMessage.decodeFromVoice(latest13s, ...); // 傳入你的解碼邏輯
    ...
    public VoiceDataMonitor getVoiceData(int duration, boolean afterDoneRemove, OnGetVoiceDataDone getVoiceDataDone) {
		Log.d(TAG, "---------------------getVoiceData() 被呼叫, afterDoneRemove=" + afterDoneRemove);
        if (isRunning) {
