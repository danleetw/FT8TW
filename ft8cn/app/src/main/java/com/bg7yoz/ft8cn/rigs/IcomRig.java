...
public class IcomRig extends BaseRig {
	...
	private static final SWRConverter.CalEntry[] ICOM_SWR_TABLE = {
		...
		new SWRConverter.CalEntry(48,  1.5f),
		new SWRConverter.CalEntry(80,  2.0f),
		new SWRConverter.CalEntry(120, 3.0f),
    ...
    public void setFreqToRig() {
        if (getConnector() != null) {
			...
			SSE_setFreqToRig ( TAG , IcomRigConstant.setOperationFrequency(ctrAddress, getCivAddress(), getFreq()) ); //設定頻率
    ...
    public void sendWaveData(Ft8Message message) {//发送音频数据到电台，用于网络方式
        if (getConnector() != null) {//把生成的具体音频数据传递到Connector，
			...
			float[] data = Ft8DecodedMessage.generateFt8(message, GeneralVariables.getBaseFrequency()
                    ,12000,GeneralVariables.isFT4);//此处icom电台发射音频的采样率是12000		
    ...
    private void showAlert() {
        ...
        if (GeneralVariables.enableswrAlcAlert && swr >= IcomRigConstant.swr_alert_max) {
            if (!swrAlert) {
                swrAlert = true;
    ...
    public void onReceiveData(byte[] data) {
		...
		SSE_receiveData( TAG ,  ""+ dataBuffer ); //⬅️收到回應
    ...
    public void readFreqFromRig() {
        if (getConnector() != null) {
			SSE_readFreqFromRig( TAG , IcomRigConstant.setReadFreq(ctrAddress, getCivAddress()) ); //➡️讀取頻率
        ...
        meterTimer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                if (isPttOn() && !oldVersion) {//当Ptt被按下去的时候测量,并且不是老版本的电台
					...
					GeneralVariables.sendToSSE(TAG , "D", "➡️sendCivData getSWRState");
                    sendCivData(IcomRigConstant.getSWRState(ctrAddress, getCivAddress()));
					
					GeneralVariables.sendToSSE(TAG , "D", "➡️sendCivData getALCState");
    ...
    public IcomRig(int civAddress,boolean newRig) {
        LogExt.d(TAG, "IcomRig: Create.");
        this.oldVersion = !newRig;
		super.SendtoSSE("➡️ setCivAddress:"+civAddress );
