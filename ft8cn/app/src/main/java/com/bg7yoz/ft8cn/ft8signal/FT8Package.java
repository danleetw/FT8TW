 ...
 * @author BGY70Z
 * @date 2023-03-20
 * 2025/7/1 避免pack_r1_p1傳入小於兩位資料而當機
 */
...
public class FT8Package {
    ...
    public static byte[] generatePack77_i4(Ft8Message message) {
        ...
        if (message.checkIsCQ()) {//如果是CQ，就把自己的呼号的哈希加上
			int[] hashes = Ft8DecodedMessage.computeHashes(fromCall);
            //hash12 = getHash12(fromCall);
			hash12 = hashes[1];
        } else {
			int[] hashes = Ft8DecodedMessage.computeHashes(toCall);
...
//            fromCall = message.callsignFrom.substring(0, message.callsignFrom.length() - 2);
    ...
    public static int pack_R1_g15(String grid4) {
        ...
        if (grid4.charAt(0) == 'R') {
			...
			try{
				...
				Log.e(TAG, "pack_R1_g15: 無法解析字串為數字: "+grid4);
				return MAXGRID4 + 1; // 作為預設選擇
			}
    ...
    public static byte pack_r1_p1(String callsign) {
		String base = (callsign != null ? callsign.trim() : "");
		
        String s;
		
		if (base.length() >= 2) {
			s = base.substring(base.length() - 2);
		}else{
			s = "  "; // 如果傳入值小於兩位則不用判斷
		}
     ...
     */
    private static String formatCallsign(String s) {
		if (s == null) s = "";
		String callsign = s.trim().toUpperCase();
		// FT8 要用固定 6 个字符（假设是 6），不足就补空格
		final int REQUIRED = 6;
		if (callsign.length() < REQUIRED) {
			callsign = String.format("%-" + REQUIRED + "s", callsign);
		}
