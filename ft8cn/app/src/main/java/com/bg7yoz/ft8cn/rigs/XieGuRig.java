    ...
    private void showAlert() {
        ...
        if (GeneralVariables.enableswrAlcAlert && swr >= IcomRigConstant.swr_alert_max) {
            if (!swrAlert) {
				...
				ToastMessage.show(String.format( GeneralVariables.getStringFromResource(R.string.swr_high_alert) , swr ,IcomRigConstant.swr_alert_max) );
            }
        } else {
            swrAlert = false;
        }
		...
		//协谷的alc值，是在指定范围之内
        //alc太高
        if (GeneralVariables.enableswrAlcAlert && (alc > IcomRigConstant.xiegu_alc_alert_max)
                ) {//网络模式下不警告ALC
            if (!alcMaxAlert) {
                alcMaxAlert = true;
        ...
        //alc太低
        if (GeneralVariables.enableswrAlcAlert && (alc < IcomRigConstant.xiegu_alc_alert_min)
                ) {//网络模式下不警告ALC
            if (!alcMinAlert) {
                alcMinAlert = true;
