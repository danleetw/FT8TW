 ...
 * @author BG7YOZ
 * 2023-03-20
 * 2025/7/2 BV6LC 修改記憶體洩漏 this.context
...
public class CallsignDatabase extends SQLiteOpenHelper {
    ...
    public static CallsignDatabase getInstance(@Nullable Context context, @Nullable String databaseName, int version) {
        if (instance == null) {
			...
			instance = new CallsignDatabase(appCtx, databaseName, null, version);
    ...
    public static synchronized void getMessagesLocation(SQLiteDatabase db, ArrayList<Ft8Message> ft8Messages ) {
        ...
        for (Ft8Message msg : messages) {
			LogExt.d(TAG, "====Country 2====From " + msg.callsignFrom+ " "+msg.i3 +" "+ msg.n3);
			
            if (msg.i3==0 && msg.n3==0) {
				LogExt.d(TAG, "自由文本，就不查了" + msg.callsignFrom+ " "+msg.i3 +" "+ msg.n3);
				continue;//如果是自由文本，就不查了
			}
            ...
            if (fromCallsignInfo != null) {
					...
					LogExt.d(TAG, "====Country 2====From " + msg.callsignFrom+ " Dx:"+msg.fromDxcc+ " ITU:"+msg.fromItu + " fromCq:"+ msg.fromCq + " CQZone:"+fromCallsignInfo.CQZone +" "+GeneralVariables.getCqZoneById(fromCallsignInfo.CQZone));
            ...
            if (toCallsignInfo != null) {
                ...
                msg.toCq = !GeneralVariables.getCqZoneById(toCallsignInfo.CQZone); // CQZone是否存在
            ...
            for (int i = 0; i < callsignInfos.size(); i++) {
                try {
                    ...
                    for (String s : calls
						...
						//Log.d(TAG, "countryId：" +i+ " callsign:"+s);
