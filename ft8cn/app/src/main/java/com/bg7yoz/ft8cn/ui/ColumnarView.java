...
public class ColumnarView extends View {
    ...
    public void setWaveData(int[] data) {
		//Log.d(TAG, "---------------setWaveData called, length=" + (data != null ? data.length : 0));
		//Log.d(TAG, "setWaveData called, getWidth=" + getWidth() + ", getHeight=" + getHeight());
		//Log.d(TAG, "visibility=" + getVisibility() + ", alpha=" + getAlpha());
        if (data == null) {
            return;
        }
        if (data.length <= 0) {
        ...
        for (int i = 0; i < data.length / 2; i++) {
			...
			int barWidth = Math.max(1, width - spacing);
			colRect.right = colRect.left + barWidth;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (w > 0 && h > 0) {
			...
			Log.e(TAG, "Invalid width or height: width=" + w + ", height=" + h);
		}
    ...
    protected void onDraw(Canvas canvas) {
		...
		if (lastBitMap == null || _canvas == null) {
			//Log.e(TAG, "!!!!!!!!Bitmap or Canvas is null, skipping onDraw.");
        ...
        for (int i = 0; i < newData.size(); i++) {
			Rect colRect = newData.get(i);
			_canvas.drawRect(colRect, paint);
            //_canvas.drawRect(newData.get(i), paint);
			//Log.d(TAG, "Draw bar " + i + " rect=" + colRect.left + "," + colRect.top + "," + colRect.right + "," + colRect.bottom);
        }
        if (drawblock) {
            for (int i = 0; i < blockData.size(); i++) {
        ...
        //canvas.drawLine(0, 0, getWidth(), getHeight(), testPaint);
		//invalidate(); BV6LC暫時移除
    }
