...
public class Yaesu3Command {
	...
	public static boolean isPOWERMeter39(Yaesu3Command command) {
        if (command.data.length() < 3) return false;
        return (command.data.charAt(0) == '0' || command.data.charAt(0) == '1'); // P1傳回來應該是000~100 所以應該是1或0
    }
	public static int getPower39(Yaesu3Command command) {
    ...
    public static int get590ALCOrSWR(Yaesu3Command command) {
		...
		// BV6LC
		//Log.d(TAG, "--SWR/ALC:"+ command.data.substring(1, 5) );
    ...
    public static boolean is590MeterSWR(Yaesu3Command command){
		//Log.d(TAG, "--Check SWR:"+command.data.length() +" cmd:"+command.data+ " 0:"+command.data.charAt(0)+ " 1:"+command.data.charAt(1)+" 2:"+command.data.charAt(2));
        if (command.data.length() < 5) return false;
