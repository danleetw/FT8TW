...
//        }
    ...
    static class GetAllQSLCallsign {
        public static void get(SQLiteDatabase db) {
			...
			String querySQL;
			int qsoFilterMode=0;
			int qsoFilterValue=0;
			
			switch (GeneralVariables.qsoFilterOpt) {
				case 0:
					qsoFilterMode=0;
					qsoFilterValue=0;
					break;
			...
			switch (qsoFilterMode) {
				case 0: // 永久(曾經通聯過)
					querySQL = "select distinct [call] from QSLTable where band=? and mode=? and station_callsign=?";
					break;
				case 1: // Mode=1 N天
					querySQL = 	"select distinct [call] from QSLTable where band=? and mode=? and station_callsign=?" +
								"and qso_date >= strftime('%Y%m%d', 'now', 'utc', '-" + qsoFilterValue + " day') " +
								"and qso_date_off >= strftime('%Y%m%d', 'now', 'utc', '-" + qsoFilterValue + " day') ";
					break;
				case 2: //HOUR=N小時之內
					querySQL = 	"select distinct [call] from QSLTable where band=? and mode=? and station_callsign=?" +
								"and ( "+
								"datetime(substr(qso_date, 1, 4) || '-' || substr(qso_date, 5, 2) || '-' || substr(qso_date, 7, 2) || ' ' || substr(time_on, 1, 2) || ':' || substr(time_on, 3, 2) || ':' || substr(time_on, 5, 2)) >= datetime('now', 'utc' , '-" + qsoFilterValue + " hour') "+
								"OR "+
								"datetime(substr(qso_date_off, 1, 4) || '-' || substr(qso_date_off, 5, 2) || '-' || substr(qso_date_off, 7, 2) || ' ' ||  substr(time_off, 1, 2) || ':' || substr(time_off, 3, 2) || ':' || substr(time_off, 5, 2)) >= datetime('now', 'utc', '-" + qsoFilterValue + " hour')"+
								")";
					break;
				default:
					throw new IllegalArgumentException("Unknown mode: " + qsoFilterMode);
			}
			//querySQL = 	"select * from qsltable where band=?";			
			Log.d(TAG,"------"+querySQL+"-----");
			...
			// 如果Debug開了，後面資料會找不到
			if (cursor != null) {
				Log.d("SQL_RESULT", "共找到筆數: " + cursor.getCount());
				...
				while (cursor.moveToNext()) {
					...
					for (String column : columnNames) {
						...
						rowLog.append(column).append("=").append(value).append("  ");
					}

					Log.d("SQL_ROW", rowLog.toString());
				}

				cursor.close();
			} else {
				Log.d("SQL_RESULT", "Cursor 為 null");
			}
			*/
            ...
            while (cursor.moveToNext()) {
                ...
                if (s != null) {
					Log.d(TAG,"篩選的Call Sign:"+s);
                    callsigns.add(s);
                }
            }
            cursor.close();
            GeneralVariables.QSL_Callsign_list = callsigns; //載入QSL紀錄
    ...
    static class GetAllConfigParameter extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (name.equalsIgnoreCase("callsign")) {
                    ...
                    if (callsign.length() > 0) {
						hashes = Ft8DecodedMessage.computeHashes(callsign);
						...
						Ft8Message.hashList.addHash(hashes[0], callsign);
						Ft8Message.hashList.addHash(hashes[1], callsign);
						Ft8Message.hashList.addHash(hashes[2], callsign);
                        ...
                        if (callsign.contains("/")) {
							...
							Ft8Message.hashList.addHash(hashes[0], shortCallsign);
                            Ft8Message.hashList.addHash(hashes[1], shortCallsign);
                            Ft8Message.hashList.addHash(hashes[2], shortCallsign);
                        }
                    }
                }
                ...
                if (name.equalsIgnoreCase("bandFreq")) {
                    ...
                    GeneralVariables.bandListIndex = OperationBand.getIndexByFreq(GeneralVariables.band,GeneralVariables.isFT4);
                }
                ...
                //    GeneralVariables.darkModeSetting = Integer.parseInt(result) ;
                //}
				...
				if (name.equalsIgnoreCase("rigName")) {//电台型号名稱
                    GeneralVariables.rigName = result;
                }
                ...
                if (name.equalsIgnoreCase("autoFollowCQ")) {// 自動回應CQ
                    GeneralVariables.autoFollowCQ = (result.equals("") || result.equals("1"));
                }
                
				/*
				if (name.equalsIgnoreCase("autoAddFollow")) {//自動儲存FOLLOW
                    GeneralVariables.autoAddFollow = (result.equals("") || result.equals("1"));
                }*/
                ...
                if (name.equalsIgnoreCase("volumeValue")) {//输出音量大小
                   // GeneralVariables.volumePercent = result.equals("") ? 1.0f : Float.parseFloat(result) / 100f;
					...
					try {
						GeneralVariables.volumePercent = (result.equals("")) ? 0.8f : Float.parseFloat(result) / 100f;
					} catch (Exception e) {
					GeneralVariables.volumePercent = 0.8f; // 防呆，預設80%
					}
                ...
                //}
				if (name.equalsIgnoreCase("deepMode")) {//是不是深度解码模式
                    GeneralVariables.deepDecodeMode =Integer.parseInt(result);
                }
				
				if (name.equalsIgnoreCase("ft4ft8Mode")) {//是不是深度解码模式
                    GeneralVariables.isFT4 = Integer.parseInt(result)==0 ? true : false;
					Log.d(TAG, "ft4ft8:"+GeneralVariables.isFT4);
					GeneralVariables.setIsFT4(GeneralVariables.isFT4);
                }
				...
				if (name.equalsIgnoreCase("qrzUploadOpt")) {
					GeneralVariables.qrzUploadOpt = Integer.parseInt(result);
				}
				...
				// sotamat
				if (name.equalsIgnoreCase("sotamat")) {
					GeneralVariables.sotamat = result;
				}
				
				if (name.equalsIgnoreCase("qsoCnt")) { // 讀入QSO次數
                    GeneralVariables.qsoCnt = result.equals("") ? 0 : Integer.parseInt(result);
                }

				if (name.equalsIgnoreCase("qsoFilterOpt")) { // 讀入QSO過濾器參數
                    GeneralVariables.qsoFilterOpt = result.equals("") ? 0 : Integer.parseInt(result);
                }

				
				if (name.equalsIgnoreCase("cqOrderOpt")) { // 讀入CQ優先方案參數
                    GeneralVariables.autoFollowCQOrder = result.equals("") ? 0 : Integer.parseInt(result);
                }
				
				// SwrAlert
				if (name.equalsIgnoreCase("swrAlert")) {
					GeneralVariables.enableswrAlcAlert = result.equals("1");
				}
            ...
            GetAllQSLCallsign.get(db);//获取通联过的呼号 讀入所有通聯過呼號QSL
