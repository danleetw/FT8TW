...
public class GridTrackerMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		*/
		...
		// 全螢幕顯示 (取代 FLAG_FULLSCREEN) 2025/11/08
		WindowCompat.setDecorFitsSystemWindows(getWindow(), false);

		WindowInsetsControllerCompat controller =
				new WindowInsetsControllerCompat(getWindow(), getWindow().getDecorView());

		controller.hide(WindowInsetsCompat.Type.systemBars()); // 隱藏狀態列與導覽列
		...
		// 優先取用 MainActivity 已初始化的實例（含電台連線），避免建立無連線的新實例
		mainViewModel = MainViewModel.getMainInstance();
        ...
        binding.transmittingMessageTextView.setAnimation(AnimationUtils.loadAnimation(this
                , R.anim.view_blink));
        //观察发射的状态 - 使用 coreInitialized 延遲綁定，防止 ft8TransmitSignal 尚未初始化
        mainViewModel.coreInitialized.observe(this, ready -> {
            if (Boolean.TRUE.equals(ready) && mainViewModel.ft8TransmitSignal != null) {
    ...
    private void InitFloatView() {
        ...
        transButton = floatView.addButton(R.id.grid_tracker_trans, "grid_tracker_trans"
                ...
                , new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        // ✅ 防止 ft8TransmitSignal 尚未初始化
                        if (mainViewModel.ft8TransmitSignal == null) return;
                        //如果
                        if (!mainViewModel.ft8TransmitSignal.isActivated()) {
                            //mainViewModel.ft8TransmitSignal.restTransmitting();
							mainViewModel.ft8TransmitSignal.restTransmittingAndChangeSeq();
                        }
        ...
        //显示当前目标呼号和发射状态 - 使用 coreInitialized 延遲綁定
        mainViewModel.coreInitialized.observe(this, ready -> {
            if (Boolean.TRUE.equals(ready) && mainViewModel.ft8TransmitSignal != null) {
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        ...
        GeneralVariables.resetLaunchSupervision();//复位自动监管
        // ✅ 防止 ft8TransmitSignal 尚未初始化
        if (mainViewModel.ft8TransmitSignal == null) return super.onContextItemSelected(item);
