    ...
    static class DistanceCount extends AsyncTask<Void,Void,Void>{
        ...
        private String getQSLInfo(String grid){
            ...
            while (cursor.moveToNext()){
				...
				// 檢查 callsignInfo 是否為 null
				if(callsignInfo==null)
					//Log.d(TAG, "!!!!!!!!!!!!!==="+call);
					ToastMessage.show(String.format("Call sign %s country is missing",call));
				String callInfoStr = (callsignInfo != null && callsignInfo.toString() != null) ? 
                     callsignInfo.toString() : "ERR"; //BV6LC
					 

				if (breakLine>0) result.append("\n");
				 result.append(String.format("%s %s(%s) %s\n %s", call, freq, band
						 ,gridsquare,callInfoStr ));
				 breakLine++;
            }
