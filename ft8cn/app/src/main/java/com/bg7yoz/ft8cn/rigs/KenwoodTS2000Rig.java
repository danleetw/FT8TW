...
public class KenwoodTS2000Rig extends BaseRig {
	...
	private void readPower(){
        if (getConnector() != null) {
            ...
            getConnector().sendData( KenwoodTK90RigConstant.setReadPower() );
        }
    }
    ...
    public void onReceiveData(byte[] data) {
        ...
        if (!s.contains("\r"))
        {
			...
			else if (cmd.equalsIgnoreCase("PC")){ // 2025/11/23 新增讀取Power
				super.radioResponsed();//有回應
				float power = Float.parseFloat(yaesu3Command.getData());
