...
public class BaseRigConnector {
	...
	public synchronized void sendData(byte[] data){
		// 1️⃣ 先印到 SSE（標示為 TX）
		com.bg7yoz.ft8cn.rigs.BaseRig.SendtoSSE("➡️ TX [" 
			+ com.bg7yoz.ft8cn.rigs.BaseRig.bytesToHex(data) + "]"+data);

		// 2️⃣ 再呼叫真正送出的方法（由子類別實作）
		onSendData(data);
	}
	
	// 這是子類別的實際送出方法（USB、Bluetooth、Network）
	protected void onSendData(byte[] data){
		// 由 CableConnector / BluetoothConnector / NetworkConnector 覆寫
	}
