    ...
    public CableConnector(Context context, CableSerialPort.SerialPort serialPort, int baudRate
        ...
        cableSerialPort.ioListener = new SerialInputOutputManager.Listener() {
            ...
            public void onRunError(Exception e) {
                ...
                getOnConnectorStateChanged().onRunError("Serial port disconnected." + e.getMessage());
            }
        };
        //connect();
    }
...
//    }
    ...
    public void disconnect() {
		...
		if (cableSerialPort != null) {
			try {
				cableSerialPort.disconnect();  // 照你原本的架構
			} catch (Exception e) {
				Log.e("CableConnector", "Error during cableSerialPort.disconnect(): " + e.getMessage());
			}
		}
		...
		try {
			...
			Log.e("CableConnector", "Error during super.disconnect(): " + e.getMessage());
		}
