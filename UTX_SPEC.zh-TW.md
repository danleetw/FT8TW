...
以 Normal 模式（15 秒／frame）傳送 **20 個字** 為例：

| 內容 | 每 frame 承載 | 所需 frame 數 | 約需時間 |
|---|---|---|---|
| 英文（Mode A，速度與 legacy 相同） | ~15.3 字 | 2 | 30 秒 |
| 中文常用字（UTX tier1） | ~5.3 字 | 4 | 60 秒 |
| 諺文／次常用字（UTX tier2） | ~4.3 字 | 5 | 75 秒 |
| 西里爾等拼音文字（UTX 視窗） | ~8.5 字 | 3 | 45 秒 |
| 對照：同一段中文改用裸 UTF-8（不經 UTX 編碼） | ~2.9 字 | 7 | 105 秒 |

同樣 20 個中文字，UTX（tier1）約 60 秒可傳完，比裸 UTF-8 的 105 秒快約 **1.75 倍**；
英文內容速度則與原本的 legacy JS8 完全相同，不受 UTX 影響。Fast／Turbo／Slow
子模式的 frame 時長不同（10／6／30 秒），上表數字依比例縮放即可。
