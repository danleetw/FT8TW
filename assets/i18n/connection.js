...
en: {
  ...
  conn_audio_note: 'The app detects USB sound cards being plugged and unplugged and follows automatically (Android 6 and later). If that page still shows the built-in microphone after you plug one in, press <strong>Re-detect</strong> — no need to restart the app. The page also distinguishes the case where a USB audio device was found but the system refused the routing request, so recording is still coming from the built-in microphone, and says so outright. <strong>Two cases deliberately do not switch:</strong> network-connected radios (FlexRadio, ICOM, Xiegu) take their audio over the network rather than the microphone, so plugging a sound card in or out does not affect them; and while transmitting the switch is deferred, for up to 30 seconds. That page also carries six diagnostic fields showing what the recorder is really capturing — see <a href="troubleshoot.html">Troubleshooting</a> for how to read them.',
...
'zh-TW': {
  ...
  conn_usb_note: '幾件值得先知道的事：裝置清單只在<strong>偵測到序列埠裝置、連接方式為 USB、且尚未連線</strong>時才會出現在解碼畫面上方；只有<strong>剛好一個序列埠裝置</strong>時才會在啟動時自動連線，接了兩條以上仍需手動點一次。清單會濾掉音效卡、鍵盤、隨身碟這類明確不可能是序列埠的裝置，所以 Digirig 只會出現序列埠那一項。多埠的序列晶片——例如 Yaesu FTX-1 內建的 Silicon Labs CP2105——每個埠的廠商、型號與序號完全相同，因此每一列後面會標上 <code>[Port n/total]</code> 供辨別，單埠裝置則維持原樣不標。以 FTX-1 為例，Port 1 是 Enhanced 介面（CAT-1，頻率與模式），Port 2 是 Standard（CAT-2）。授權對話框按下「允許」後會直接連線，不必再點第二次；拔掉線會主動斷開，重新插上約一兩秒自動接回。<strong>手機必須自己切換成 USB 主機（OTG）</strong>，這與插在電腦上不同——同一條線在電腦上正常、手機卻連清單都沒有時，請看<a href="troubleshoot.html">「問題排除」</a>的 USB 一節。',
  ...
  conn_audio_note: 'App 會自動偵測 USB 音效卡的插拔並跟著切換（Android 6 以上）。若插上之後那一頁顯示的仍是內建麥克風，按一下<strong>重新偵測</strong>即可，不必重開 App。這一頁也會分辨「找到 USB 音訊，但系統不接受路由要求，實際仍在收內建麥克風」這種情況，並直接寫出來。<strong>有兩種情況刻意不自動切換</strong>：網路連線的電台（FlexRadio、ICOM、協谷）音訊走網路而不是麥克風，插拔音效卡不會影響它們；發射中會延後處理，最多讓路 30 秒。另外，那一頁還有六行診斷細節可以看出「錄音物件現在真的在收什麼」，判讀方式見<a href="troubleshoot.html">「問題排除」</a>。',
...
'zh-CN': {
  ...
  conn_usb_note: '几件值得先知道的事：设备列表只在<strong>检测到串口设备、连接方式为 USB、且尚未连接</strong>时才会出现在解码画面上方；只有<strong>刚好一个串口设备</strong>时才会在启动时自动连接，接了两条以上仍需手动点一次。列表会滤掉声卡、键盘、U 盘这类明确不可能是串口的设备，所以 Digirig 只会出现串口那一项。多口的串口芯片——例如 Yaesu FTX-1 内置的 Silicon Labs CP2105——每个口的厂商、型号与序列号完全相同，因此每一列后面会标上 <code>[Port n/total]</code> 供辨别，单口设备则维持原样不标。以 FTX-1 为例，Port 1 是 Enhanced 接口（CAT-1，频率与模式），Port 2 是 Standard（CAT-2）。授权对话框按下「允许」后会直接连接，不必再点第二次；拔掉线会主动断开，重新插上约一两秒自动接回。<strong>手机必须自己切换成 USB 主机（OTG）</strong>，这与插在电脑上不同——同一条线在电脑上正常、手机却连列表都没有时，请看<a href="troubleshoot.html">「问题排查」</a>的 USB 一节。',
  ...
  conn_audio_note: 'App 会自动检测 USB 声卡的插拔并跟着切换（Android 6 以上）。若插上之后那一页显示的仍是内置麦克风，按一下<strong>重新检测</strong>即可，不必重开 App。这一页也会分辨「找到 USB 音频，但系统不接受路由请求，实际仍在收内置麦克风」这种情况，并直接写出来。<strong>有两种情况刻意不自动切换</strong>：网络连接的电台（FlexRadio、ICOM、协谷）音频走网络而不是麦克风，插拔声卡不会影响它们；发射中会延后处理，最多让路 30 秒。另外，那一页还有六行诊断细节可以看出「录音对象现在真的在收什么」，判读方式见<a href="troubleshoot.html">「问题排查」</a>。',
...
'ja': {
  ...
  conn_usb_note: '先に知っておくとよい点がいくつかあります。デバイス一覧がデコード画面の上に出るのは、<strong>シリアルデバイスを検出していて、接続方式が USB で、まだ接続していない</strong>ときだけです。起動時の自動接続は<strong>シリアルデバイスがちょうど 1 台</strong>のときに限られ、2 台以上つないでいる場合は手動で選びます。一覧はサウンドカード・キーボード・USB メモリーなど、シリアルポートではありえないものを除外するので、Digirig はシリアルの項目だけが表示されます。ポートが複数あるシリアルチップ——たとえば Yaesu FTX-1 内蔵の Silicon Labs CP2105——はベンダー名・型番・シリアル番号がどれも同じ行になるため、各行の末尾に <code>[Port n/total]</code> を付けて区別できるようにしてあります（単ポートの機器には付きません）。FTX-1 では Port 1 が Enhanced（CAT-1、周波数とモード）、Port 2 が Standard（CAT-2）です。許可ダイアログで「許可」を押すとそのまま接続され、二度目のタップは不要です。ケーブルを抜くとすぐ切断し、挿し直すと 1〜2 秒で再接続します。<strong>スマートフォン側が USB ホスト（OTG）に切り替わる必要があります</strong>。これはパソコンにつなぐのとは別のことで、同じケーブルがパソコンでは動くのにスマートフォンでは一覧すら出ない場合は、<a href="troubleshoot.html">「トラブルシューティング」</a>の USB の節をご覧ください。',
  ...
  conn_audio_note: 'USB サウンドカードの抜き差しはアプリが自動的に検出して切り替えます（Android 6 以降）。接続してもそのページに内蔵マイクと表示されたままなら、<strong>再検出</strong>を押せば切り替わり、アプリの再起動は不要です。「USB オーディオは見つかったが、システムが経路の変更を受け付けず、実際には内蔵マイクから録音している」という状態もこのページははっきり区別して表示します。<strong>意図的に切り替えない場合が 2 つあります</strong>。ネットワーク接続の無線機（FlexRadio、ICOM、Xiegu）は音声がネットワーク経由なので、USB サウンドカードの抜き差しの影響を受けません。また送信中は切り替えを保留し、最大 30 秒まで待ちます。なお、このページには「録音オブジェクトが今、実際に何を拾っているか」が分かる 6 行の診断項目もあります。読み方は<a href="troubleshoot.html">「トラブルシューティング」</a>を参照してください。',
...
'ru': {
  ...
  conn_usb_steps: `
    <ol>
      ...
      <li>Выберите <strong>модель трансивера</strong> из списка поддерживаемых (более 80).</li>
  ...
  conn_usb_note: 'Несколько вещей, которые стоит знать заранее. Список устройств появляется над экраном декодирования только тогда, когда <strong>последовательное устройство обнаружено, тип подключения — USB, а трансивер ещё не подключён</strong>. Автоматическое подключение при запуске происходит, только если последовательное устройство <strong>ровно одно</strong>; при двух и более выбирать придётся вручную. Из списка отфильтровываются звуковые карты, клавиатуры и флешки, которые последовательными портами быть не могут, поэтому от Digirig остаётся только последовательная строка. Микросхемы с несколькими портами — например Silicon Labs CP2105 внутри Yaesu FTX-1 — дают строки с одинаковыми производителем, моделью и серийным номером, поэтому к каждой добавляется метка <code>[Port n/total]</code>; у однопортовых устройств её нет. У FTX-1 порт 1 — это интерфейс Enhanced (CAT-1, частота и режим), порт 2 — Standard (CAT-2). Нажатие «Разрешить» в диалоге прав сразу выполняет подключение, второе касание не нужно; при отсоединении кабеля связь разрывается немедленно, а при обратном подключении восстанавливается через секунду-другую. Учтите, что <strong>телефон должен сам перейти в режим USB-хоста (OTG)</strong> — это не то же самое, что подключение к компьютеру. Если тот же кабель работает на компьютере, а на телефоне список вообще не появляется, см. раздел про USB в <a href="troubleshoot.html">«Устранении неполадок»</a>.',
  ...
  conn_audio_note: 'Приложение само определяет подключение и отключение звуковых карт USB и переключается за ними (Android 6 и новее). Если после подключения там по-прежнему показан встроенный микрофон, нажмите <strong>Определить заново</strong> — перезапускать приложение не нужно. Эта же страница отдельно указывает случай, когда устройство USB найдено, но система отказалась перенаправить на него запись, и звук по-прежнему идёт со встроенного микрофона. <strong>Есть два случая, когда переключение намеренно не происходит:</strong> трансиверы с сетевым подключением (FlexRadio, ICOM, Xiegu) получают звук по сети, а не с микрофона, поэтому подключение звуковой карты на них не влияет; а во время передачи переключение откладывается — не более чем на 30 секунд. Кроме того, на той же странице есть шесть строк диагностики, показывающих, что записывается на самом деле прямо сейчас; как их читать, см. <a href="troubleshoot.html">«Устранение неполадок»</a>.',
...
'pl': {
  ...
  conn_usb_note: 'Kilka rzeczy, które warto wiedzieć na wstępie. Lista urządzeń pojawia się nad ekranem dekodowania tylko wtedy, gdy <strong>wykryto urządzenie szeregowe, typ połączenia to USB, a radio nie jest jeszcze połączone</strong>. Automatyczne połączenie przy starcie następuje tylko wtedy, gdy urządzenie szeregowe jest <strong>dokładnie jedno</strong>; przy dwóch lub więcej trzeba wybrać ręcznie. Lista odfiltrowuje karty dźwiękowe, klawiatury i pendrive’y, które portami szeregowymi być nie mogą, więc z Digiriga zostaje tylko pozycja szeregowa. Układy z więcej niż jednym portem — na przykład Silicon Labs CP2105 wewnątrz Yaesu FTX-1 — dają wiersze o identycznym producencie, modelu i numerze seryjnym, dlatego każdy z nich jest oznaczony jako <code>[Port n/total]</code>; urządzenia jednoportowe pozostają bez oznaczenia. W FTX-1 Port 1 to interfejs Enhanced (CAT-1, częstotliwość i tryb), a Port 2 to Standard (CAT-2). Naciśnięcie „Zezwól" w oknie uprawnień od razu łączy, drugie dotknięcie nie jest potrzebne; odłączenie kabla natychmiast rozłącza, a ponowne podłączenie przywraca połączenie po sekundzie lub dwóch. Pamiętaj, że <strong>telefon musi sam przejść w tryb hosta USB (OTG)</strong>, co nie jest tym samym co podłączenie do komputera — jeśli ten sam kabel działa na komputerze, a w telefonie lista w ogóle się nie pojawia, zajrzyj do sekcji o USB w <a href="troubleshoot.html">„Rozwiązywaniu problemów"</a>.',
  ...
  conn_audio_note: 'Aplikacja sama wykrywa podłączenie i odłączenie kart dźwiękowych USB i przełącza się za nimi (Android 6 i nowszy). Jeśli po podłączeniu nadal widnieje tam mikrofon wbudowany, naciśnij <strong>Wykryj ponownie</strong> — restart aplikacji nie jest potrzebny. Ta sama strona osobno wskazuje sytuację, w której urządzenie USB zostało znalezione, ale system odmówił przekierowania na nie nagrywania, więc dźwięk nadal pochodzi z mikrofonu wbudowanego. <strong>W dwóch sytuacjach przełączenie celowo nie następuje:</strong> radia z połączeniem sieciowym (FlexRadio, ICOM, Xiegu) pobierają dźwięk przez sieć, a nie z mikrofonu, więc podłączanie karty ich nie dotyczy; a w trakcie nadawania przełączenie jest odraczane, maksymalnie o 30 sekund. Na tej samej stronie jest ponadto sześć wierszy diagnostyki pokazujących, co naprawdę jest w tej chwili nagrywane; jak je czytać, opisuje <a href="troubleshoot.html">„Rozwiązywanie problemów"</a>.',
...
'es': {
  ...
  conn_usb_note: 'Conviene saber algunas cosas de antemano. La lista de dispositivos aparece sobre la pantalla de decodificación solo cuando <strong>se ha detectado un dispositivo serie, el tipo de conexión es USB y todavía no hay equipo conectado</strong>. La conexión automática al arrancar se produce únicamente si hay <strong>exactamente un dispositivo serie</strong>; con dos o más hay que elegir a mano. La lista descarta tarjetas de sonido, teclados y memorias USB, que no pueden ser puertos serie, de modo que de un Digirig solo queda su entrada serie. Los chips con más de un puerto —por ejemplo el Silicon Labs CP2105 del Yaesu FTX-1— producen filas con idéntico fabricante, modelo y número de serie, así que cada una se etiqueta como <code>[Port n/total]</code> para distinguirlas; los dispositivos de un solo puerto quedan sin etiqueta. En el FTX-1, el puerto 1 es la interfaz Enhanced (CAT-1, frecuencia y modo) y el puerto 2 la Standard (CAT-2). Pulsar «Permitir» en el diálogo de permisos conecta directamente, sin necesidad de un segundo toque; desconectar el cable corta la conexión al instante y volver a enchufarlo la restablece en uno o dos segundos. Ten en cuenta que <strong>el teléfono debe pasar por sí mismo a modo anfitrión USB (OTG)</strong>, que no es lo mismo que conectarlo a un ordenador: si el mismo cable funciona en el ordenador y en el teléfono no aparece ninguna lista, consulta la sección de USB en <a href="troubleshoot.html">«Solución de problemas»</a>.',
  ...
  conn_audio_note: 'La aplicación detecta por sí sola cuándo se conecta o desconecta una tarjeta de sonido USB y cambia en consecuencia (Android 6 o posterior). Si tras conectarla esa página sigue mostrando el micrófono integrado, pulsa <strong>Volver a detectar</strong>; no hace falta reiniciar la aplicación. Esa misma página distingue además el caso en que se encontró un dispositivo USB pero el sistema rechazó encaminar la grabación hacia él, de modo que se sigue grabando del micrófono integrado. <strong>Hay dos casos en los que deliberadamente no se cambia:</strong> los equipos conectados por red (FlexRadio, ICOM, Xiegu) toman el audio por la red y no por el micrófono, así que conectar o quitar una tarjeta no les afecta; y durante la transmisión el cambio se pospone, hasta 30 segundos. Esa página incluye además seis líneas de diagnóstico que muestran qué se está grabando realmente en este momento; cómo leerlas se explica en <a href="troubleshoot.html">«Solución de problemas»</a>.',
...
'el': {
  ...
  conn_usb_steps: `
    <ol>
      ...
      <li>Επιλέξτε το <strong>μοντέλο πομποδέκτη</strong> από τη λίστα υποστηριζόμενων (άνω των 80).</li>
  ...
  conn_usb_note: 'Μερικά πράγματα που αξίζει να ξέρετε από την αρχή. Η λίστα συσκευών εμφανίζεται πάνω από την οθόνη αποκωδικοποίησης μόνο όταν <strong>έχει εντοπιστεί σειριακή συσκευή, ο τύπος σύνδεσης είναι USB και δεν έχει συνδεθεί ακόμη πομποδέκτης</strong>. Η αυτόματη σύνδεση κατά την εκκίνηση γίνεται μόνο όταν υπάρχει <strong>ακριβώς μία</strong> σειριακή συσκευή· με δύο ή περισσότερες επιλέγετε εσείς. Η λίστα φιλτράρει κάρτες ήχου, πληκτρολόγια και στικάκια USB, που δεν μπορούν να είναι σειριακές θύρες, οπότε από ένα Digirig μένει μόνο η σειριακή καταχώριση. Τα ολοκληρωμένα με περισσότερες από μία θύρες — για παράδειγμα το Silicon Labs CP2105 μέσα στο Yaesu FTX-1 — δίνουν γραμμές με ίδιο κατασκευαστή, μοντέλο και σειριακό αριθμό, γι᾽ αυτό η καθεμία σημειώνεται ως <code>[Port n/total]</code>· οι συσκευές μίας θύρας μένουν χωρίς σήμανση. Στο FTX-1 η θύρα 1 είναι η διεπαφή Enhanced (CAT-1, συχνότητα και τρόπος) και η θύρα 2 η Standard (CAT-2). Πατώντας «Να επιτραπεί» στον διάλογο αδειών η σύνδεση γίνεται αμέσως, χωρίς δεύτερο πάτημα· η αποσύνδεση του καλωδίου διακόπτει τη σύνδεση ακαριαία και η επανασύνδεση αποκαθίσταται σε ένα-δύο δευτερόλεπτα. Σημειώστε ότι <strong>το τηλέφωνο πρέπει να περάσει μόνο του σε λειτουργία USB host (OTG)</strong>, που δεν είναι το ίδιο με τη σύνδεση σε υπολογιστή — αν το ίδιο καλώδιο δουλεύει στον υπολογιστή αλλά στο τηλέφωνο δεν εμφανίζεται καθόλου λίστα, δείτε την ενότητα για το USB στην <a href="troubleshoot.html">«Αντιμετώπιση προβλημάτων»</a>.',
  ...
  conn_audio_note: 'Η εφαρμογή εντοπίζει μόνη της τη σύνδεση και αποσύνδεση καρτών ήχου USB και ακολουθεί (Android 6 και νεότερο). Αν μετά τη σύνδεση εξακολουθεί να εμφανίζεται το ενσωματωμένο μικρόφωνο, πατήστε <strong>Νέος εντοπισμός</strong> — δεν χρειάζεται επανεκκίνηση. Η ίδια σελίδα ξεχωρίζει επίσης την περίπτωση όπου βρέθηκε συσκευή USB αλλά το σύστημα αρνήθηκε να δρομολογήσει σε αυτήν την εγγραφή, οπότε ο ήχος συνεχίζει να έρχεται από το ενσωματωμένο μικρόφωνο. <strong>Σε δύο περιπτώσεις η εναλλαγή σκόπιμα δεν γίνεται:</strong> οι πομποδέκτες με σύνδεση δικτύου (FlexRadio, ICOM, Xiegu) παίρνουν τον ήχο από το δίκτυο και όχι από το μικρόφωνο, οπότε δεν επηρεάζονται· και κατά την εκπομπή η εναλλαγή αναβάλλεται, το πολύ για 30 δευτερόλεπτα. Η ίδια σελίδα διαθέτει επιπλέον έξι γραμμές διαγνωστικών που δείχνουν τι ηχογραφείται πραγματικά αυτή τη στιγμή· για το πώς διαβάζονται δείτε την <a href="troubleshoot.html">«Αντιμετώπιση προβλημάτων»</a>.',
