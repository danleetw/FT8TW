...
en: {
  ...
  set_profile_caption: 'The profile row that appears at the top of every page: the dropdown switches profile, and the three icons add, rename and delete. Rows with a purple stripe on the left edge (callsign, grid, band) follow the profile; Time offset has no stripe, so it is shared by all profiles. (Screenshot shows the Traditional Chinese interface.)',
...
'ja': {
  ...
  set_profile_caption: 'どのページの最上部にもあるプロファイル行。ドロップダウンでプロファイルを切り替え、右の 3 つのアイコンは追加・名前の変更・削除です。左端に紫色の帯がある項目（自局コール、自局GL、運用バンド）はプロファイルごとに保存され、帯のない「時刻ずれ」は全プロファイル共通です。（スクリーンショットは繁体字中国語の画面です。）',
...
'pl': {
  ...
  set_profile_caption: 'Wiersz profilu widoczny u góry każdej strony: lista rozwijana przełącza profil, a trzy ikony dodają, zmieniają nazwę i usuwają. Wiersze z fioletowym paskiem po lewej (znak, kwadrat siatki, częstotliwość) podążają za profilem; „Przesunięcie czasu" paska nie ma, więc jest wspólne dla wszystkich profili. (Zrzut ekranu przedstawia interfejs w języku chińskim tradycyjnym.)',
